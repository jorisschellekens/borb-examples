...
## Creating a patient intake document
...
In this example we're going to create a beautiful product flyer.
...
![enter image description here](img/snippet_10_13.png)

Now that we have a blank canvas, we can start adding to it.
For our product flyer, we're going to add some artwork using the `LineArt` class:

```python3
# snippet_10_14.ipynb
import typing
import random
from borb.pdf import Color, Document, LineArt, Page, PDF, X11Color
...
# build spectrum of (analogous) colors
C: typing.List[Color] = [X11Color.PRUSSIAN_BLUE]
c: Color = C[0]
for _ in range(0, 5):
    c = c.lighter()
    C += [c]
...
# add artwork
W: int = page.get_size()[0]
H: int = page.get_size()[1]
S: int = 5
for x in range(W // 10, W - W // 10, S):

    # horizontal top
    for y in range(H - H // 10, H - H // 10 + S * 4, S):
        c: Color = random.choice(C)
        LineArt.square(fill_color=c, stroke_color=c).scale_to_fit(
            width=S, height=S
        ).paint(available_space=(x, y, S, S), page=page)
...
![enter image description here](img/snippet_10_14.png)

We've used a double-nested for-loop to add a (partial) border consisting of squares (all in a different gradient of `X11Color.PRUSSIAN_BLUE`).
We can now extend that code to finish the border:

```python3
# snippet_10_15.ipynb
import typing
import random
from borb.pdf import Color, Document, Image, LineArt, Page, PDF, X11Color
...
for x in range(W // 10, W - W // 10, S):
    ...
    # horizontal bottom
    for y in range(H // 10 - S * 4, H // 10, S):
        c: Color = random.choice(C)
        LineArt.square(fill_color=c, stroke_color=c).scale_to_fit(
            width=S, height=S
        ).paint(available_space=(x, y, S, S), page=page)

for y in range(H // 10, H - H // 10, S):

    # vertical left
    for x in range(W // 10, W // 10 + S * 4, S):
        c: Color = random.choice(C)
        LineArt.square(fill_color=c, stroke_color=c).scale_to_fit(
            width=S, height=S
        ).paint(available_space=(x, y, S, S), page=page)
    ...
    # vertical right
    for x in range(W - W // 10 - S * 4 + 3, W - W // 10 + 3, S):
        c: Color = random.choice(C)
        LineArt.square(fill_color=c, stroke_color=c).scale_to_fit(
            width=S, height=S
        ).paint(available_space=(x, y, S, S), page=page)
...
![enter image description here](img/snippet_10_15.png)

Finally, we're going to add a little effect. We're going to make the bottom border fade as it gets closer to the center of the page.
We're going to do that by randomly (depending on whether the x-coordinate is close to the middle of the page) draw/not-draw the squares.
...
import typing
import random
from borb.pdf import Document, LineArt, Page, PDF, X11Color, Color
...
# we're going to use this utility function to make some of the blocks disappear
def sometimes(p: int) -> bool:
...
![enter image description here](img/snippet_10_16.png)

Now we can add a (transpartent) product image that will leap out of the page, seemingly breaking the border we drew:

```python3
# snippet_10_17.ipynb
import typing
import random
from borb.pdf import Document, Image, LineArt, Page, PDF, X11Color, Color
...
# add product picture at the bottom of the page
Image(bytes_path_pil_image_or_url="", size=(W // 2, H // 2)).paint(
    available_space=(W // 4, -H // 4, W, H // 2), page=page
)
